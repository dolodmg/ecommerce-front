...
export async function getAllBooks() { //Obtiene la lista de todos los libros
    return await apiRequest(`${BASE_URL}/get/all`, 'GET', null, 'application/json', false);
}

export async function getBook(idProduct) { //Obtiene un libro por su ID
    return await apiRequest(`${BASE_URL}/get/${idProduct}`, 'GET', null, 'application/json', false);
}

export async function getBooksByAuthor(firstName, lastName) { //Obtiene los libros de un autor por su nombre y apellido
    return await apiRequest(`${BASE_URL}/get/by-author`, 'GET', { firstName, lastName }, 'application/json', false);
}

export async function getBooksByEditorial(editorial) { //Obtiene los libros de una editorial por su nombre
    return await apiRequest(`${BASE_URL}/get/by-editorial`, 'GET', editorial, 'application/json', false);
}

export async function getBooksByPriceRange(minPrice, maxPrice) { //Obtiene los libros en un rango de precios
...
export async function getGenres() { //Obtiene todos los géneros de libros
